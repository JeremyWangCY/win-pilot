...
export const WIN_PILOT_VISIBLE_IMAGE_HISTORY = 2
const PRUNED_IMAGE_TEXT = '[Older Win-Pilot screenshot omitted from active visual context; use the newer Win-Pilot screenshots for the current state.]'
...
function winPilotCallIdsForSession(session) {
...
function imageCandidates(session) {
  ...
  const callIds = winPilotCallIdsForSession(session)
  const candidates = []
...
export function suppressDuplicateWinPilotScreenshot(result, session) {
...
export function pruneWinPilotImageHistory(session, keepImages = WIN_PILOT_VISIBLE_IMAGE_HISTORY) {
